...
3. **Mapeamento e Persistência**: Cada curso retornado é mapeado para um `CourseUnit` e salvo via `IEducationRepository`.
...
- `IEducationRepository`: Responsável por persistir os cursos (`CourseUnit`).
